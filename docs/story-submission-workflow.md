...
3.  **Configure Metadata**:
    ...
    *   *Note:* For Production, the data is in the `vercel` db. For local, the data is in the `postgres` db.
...
5.  **Test Locally**:
    ...
    *   *Note:* You must use local data for testing. If it works, update the script to target production data.
6.  **Finalize**:
    *   Save the file.
