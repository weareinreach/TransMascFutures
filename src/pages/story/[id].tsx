...
import {
	IndividualStory,
	type IndividualStoryProps,
} from "~/layouts/IndividualStory/IndividualStory";
...
const Story = () => {
	...
	const { data, isLoading } = api.story.getStoryById.useQuery({
		id: router.query.id ?? "",
		locale,
	});
	...
	const randomImage = data.categories.at(
		Math.floor(Math.random() * data.categories.length),
	)?.category?.image;
