...
const Survey = () => {
	...
	const okToProceed = useMemo(() => {
		...
		if (activeStep === 1) {
			return ["q4", "q5", "q6", "q7", "q7other", "q8", "q9"].every((q) =>
				form.isValid(q),
			);
		}
	...
	const ControlButtons = () => {
		...
		return (
			...
			>
				<Button
					onClick={prevStep}
					style={isFirst || isLast ? { opacity: 0 } : undefined}
					variant="secondary"
				>
					{t("back")}
				</Button>
	...
	const stateSelectOptions = useMemo(
		() =>
			stateOptions.map(({ value, labelEN, labelES }) => ({
				value,
				...
				>
					<Stepper.Step label={t("survey-form.step1")}>
						<Stack>
							...
							<Checkbox.Group
								...
								label={t("survey-form.q1")}
								required
							>
								{Object.entries(
									t("survey-form.q1-opts", { returnObjects: true }),
								).map(([key, value], i) => (
							...
							<Checkbox.Group
								{...form.getInputProps("q2")}
								label={t("survey-form.q2")}
								required
							>
								{Object.entries(
									t("survey-form.q2-opts", { returnObjects: true }),
								).map(([key, value], i) => (
									<Checkbox value={key.substring(3)} label={value} key={i} />
								))}
							</Checkbox.Group>
					...
					<Stepper.Step label={t("survey-form.step2")}>
						<Stack>
							...
							<TextInput
								label={<Trans i18nKey="survey-form.q4" />}
							...
							>
								{Object.entries(
									t("survey-form.q5-opts", { returnObjects: true }),
								).map(([key, value]) => (
							...
							<Stack spacing={0}>
								<Checkbox.Group
									...
									label={t("survey-form.q7")}
									required
								>
									{Object.entries(
										t("survey-form.q7-opts", { returnObjects: true }),
									).map(([key, value], i) => (
										<Checkbox value={key.substring(3)} label={value} key={i} />
									))}
								</Checkbox.Group>
							...
							<Textarea
								label={t("survey-form.q8")}
								{...form.getInputProps("q8")}
								required
							/>
						...
						<Stack>
							<Stack spacing={0}>
								...
								<Checkbox.Group
									...
									label={
										<Trans
											i18nKey="survey-form.q10"
											components={{ em: <em>.</em> }}
										/>
									}
								>
									{Object.entries(
										t("survey-form.q10-opts", { returnObjects: true }),
									).map(([key, value], i) => (
										<Checkbox value={key.substring(3)} label={value} key={i} />
									))}
								</Checkbox.Group>
							...
							<Stack spacing={0}>
								<Checkbox.Group
									...
									label={
										<Trans
											i18nKey="survey-form.q12"
											components={{ em: <em>.</em> }}
										/>
									}
								>
									{Object.entries(
										t("survey-form.q11-opts", { returnObjects: true }),
									).map(([key, value], i) => (
										<Checkbox value={key.substring(3)} label={value} key={i} />
									))}
								</Checkbox.Group>
							...
							<Stack spacing={0}>
								...
								>
									{Object.entries(
										t("survey-form.q12-opts", { returnObjects: true }),
									).map(([key, value], i) => (
										<Checkbox
											...
											disabled={
												...
												form.values?.q12?.some((x) => x === "no-answer")
											}
										/>
									))}
								</Checkbox.Group>
							...
							<Radio.Group
								{...form.getInputProps("q13")}
								label={t("survey-form.q13")}
							>
								{Object.entries(
									t("survey-form.q13-opts", { returnObjects: true }),
								).map(([key, value], i) => (
							...
							<Checkbox.Group
								...
								label={
									<Trans
										i18nKey="survey-form.q15"
										components={{ em: <em>.</em> }}
									/>
								}
							>
								{Object.entries(
									t("survey-form.q15-opts", { returnObjects: true }),
								).map(([key, value], i) => (
									<Checkbox
										value={key.substring(3)}
										label={value}
										...
										disabled={
											...
											(key.substring(3) !== "none" &&
												form.values?.q15?.some((x) => x === "none"))
										}
									/>
								))}
							</Checkbox.Group>
					...
					<Stepper.Step label={t("survey-form.step4")}>
						<AspectRatio
							ratio={ShareWide.width / ShareWide.height}
							w="100%"
							mx="auto"
							mb={40}
						>
							<Image
								src={ShareWide}
								alt="Person holding a Trans flag overhead in a downtown area."
								fill
							/>
						</AspectRatio>
